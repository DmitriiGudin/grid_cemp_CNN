...
def save_mean_std (spectrum, T_EFF, LOG_G, FE_H, C_FE, SN):
    ...
    # Save them to a file.
    std_filename = "/scratch365/dgudin/starnet_raw_data_SN/std/std_SN_"+str(SN)+".py"
    if os.path.isfile(std_filename):
        os.remove(std_filename)
    with open(std_filename,'w') as std_file:
...
if __name__ == '__main__':
    for SN in range (5, 85+1, 1):
        print "Training the network for SN =", str(SN), "..."
        # Load the data file.
        File = h5py.File("/scratch365/dgudin/starnet_raw_data_SN/spectra_processed/spectra_processed_SN_"+str(SN)+".hdf5", 'r')
        # Calculate the means and STDs for all variables involved. Save them to a file.
        spectrum_std, T_EFF_std, LOG_G_std, FE_H_std, C_FE_std, spectrum_mean, T_EFF_mean, LOG_G_mean, FE_H_mean, C_FE_mean = save_mean_std(File['spectrum'][:], File['T_EFF'][:], File['LOG_G'][:], File['FE_H'][:], File['C_FE'][:], SN)
        ...
        batch_model.save("/scratch365/dgudin/starnet_raw_data_SN/CNN_batch/CNN_batch_SN_"+str(SN)+".hdf5")
        ...
        single_model.save("/scratch365/dgudin/starnet_raw_data_SN/CNN_single/CNN_single_SN_"+str(SN)+".hdf5")
        print "Training done."
