...
class Segment():
    ...
    def is_edge(self, which):
        ...
        if which == "left":
            self.midpoint = self.wl[0]

        elif which == "right":
            self.midpoint = self.wl[-1]
    ...
    def get_statistics(self):
        ...
        self.flux_med = np.median(self.flux[np.where((self.flux >= np.percentile(self.flux, 50)) & (self.flux <= np.percentile(self.flux, 98)))])

        ### get robust min/max estimate for the flux in segment
        self.flux_min = np.percentile(self.flux, 50)
